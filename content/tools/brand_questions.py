...
def main():
    ...
    try:
        bars = json.load(open(CAT + '/_url_bars.json'))['batches']
    ...
    for bid, rs in sorted(by.items()):
        ...
        bar = bars.get(bid, {})
        url = bar.get('domain', '')
        others = bar.get('others') or []
        from_url = sorted({r['brand_evidence'] for r in guessed if r.get('brand_evidence')})
        L += ['### %d. Batch `%s` — %d product%s' % (n, bid, len(unknown), '' if len(unknown) == 1 else 's'),
              '', ' '.join(pics) or '*(no usable image)*', '',
              '- **Address bar:** %s' % (('`%s` on %d of %d screenshots' % (url, bar.get('n', 0), bar.get('of', 0)))
                                         if url else 'not legible'),
              ...
              '- **Evidence:** %s' % (from_url[0] if from_url else
                                      ('the shop is mono-brand and every legible brand in the batch is the same'
        ...
        if others:
            L.insert(len(L) - 1,
                     '- **Careful:** this batch also shows %s in its address bar, so the clustering '
                     'has run two shops together.' % ', '.join('`%s`' % o for o in others))
